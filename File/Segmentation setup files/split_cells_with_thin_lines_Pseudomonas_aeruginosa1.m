...
%     imagesc(LL)
 ...
 ROI_Filtered=  matrix_Filtered(X1:X2,Y1:Y2);  
  ROI_Filtered(ROI3==0)=0;
    ...
    if   maxi==2 % most likely 2 cells
            ROI3=watershed_split_delta_classify(ROI3);
            matrix(X1:X2,Y1:Y2)=matrix(X1:X2,Y1:Y2)+ double(ROI3);
    end
    
        if  maxi>2 % most likely bulb of 2 and more cells
            D = bwdist(ROI2);
            DL = watershed(D);% watershed is less accurate than atershed_split_delta_classify but always do the split
            ROI3( DL == 0)=0;
