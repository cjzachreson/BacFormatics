...
if nargin==0                  
a=1;                  
b=50; % 50 estamted as the limit for the last case                  
matrix= {                  
'Select:'                  
} ;                 
else                  
...
case 10                  
if nargin==0                  
matrix(10)={  'remove segments using local maximum'}  ;                  
else         
%     matrix=double(matrix);
% matrix=make2round(matrix,strel_value)   ;    
maxima_full_filename  =char(strcat(pathname_Raw,'ch01_Filtered',filesep,Raw_filename,'_ch01_Filtered.tif'))  ;  
maxima_matrix=double(imread_cross(maxima_full_filename));
maxima_matrix =maxima_matrix ./max(max(maxima_matrix ));
maxima_matrix =uint8(maxima_matrix *255)  ;
 
 
maxima_matrix2= maxima_matrix(:) ;
maxima_matrix2(maxima_matrix2==0)=[];  
Level=graythresh(maxima_matrix2)*255*0.8;%0.8 to include more faint points 
 

maxima_matrix(  maxima_matrix<Level)=0;
...
case 20                  
if nargin==0                  
matrix(20)={  'Inverse intensity with inverted frame'}  ;                  
else   
...
%      matrix=255-matrix;
%     matrix=im2bw(matrix,0.99);
% 
% figure
% imagesc(matrix)
% load X
% hold on;plot(X(:,1),X(:,2),'r.',     'MarkerSize',7)  
end
...
case 27                  
if nargin==0                  
matrix(27)={'Remove cells touching image edges'}     ;             
else            
...
case 29                  
if nargin==0                  
matrix(29)={'clean rods for David'}     ;    
else  
     matrix=~matrix;
    matrix=bwmorph(matrix,'spur' );
     matrix=bwmorph(matrix,'clean' );
...
case 31                  
if nargin==0                  
matrix(31)={'clean by termination points'}     ;             
else                  
%   
...
case 32                  
if nargin==0                  
matrix(32)={'filter bacteria from phase contrast'}     ;             
else                  
...
L = bwlabel(matrix,4);
stats=regionprops(L,'MajorAxisLength','Area','Perimeter','Eccentricity');
MajorAxisLength=cat(1,stats.MajorAxisLength); 
Area=cat(1,stats.Area); 
...
idx1  = find(Area> 120);
idx2  = find(Area <2500);
idx3  = find(MajorAxisLength>15); 
idx4  = find(Eccentricity>0.85);  
% idx5 = find(Circularity < 0.8);   
...
for ii=1:size(s,1)
        ...
        ROI_bw=  L(X1:X2,Y1:Y2) ; 
         ...
         try
         matrix2(X1:X2,Y1:Y2)=matrix2(X1:X2,Y1:Y2)  +  patch_ROI(ROI_bw,5,8); 
    catch
...
case 33                  
if nargin==0                  
matrix(33)={' clean bacteria segments'}     ;             
else                  
...
case 35                  
if nargin==0                  
matrix(35)={'Exclusion by Eccentricity'}     ;             
else                  
L =bwlabel(matrix,4); 
...
case 36                  
if nargin==0                  
matrix(36)={'Exclusion base on majoraxis and area (parameters infile)'}     ;             
else                  
L =bwlabel(matrix); 
stats=regionprops(L,'MajorAxisLength','Area'); 
...
case 38                  
if nargin==0                  
matrix(38)={'split_cells_with_thin_lines_Pseudomonas_aeruginosa1'}     ;             
else                  
 
  full_filename  =char(strcat(pathname_Raw,'ch00_Filtered',filesep,Raw_filename,'_ch00_Filtered.tif'));   
matrix_Filtered=imread_cross(full_filename);

 matrix=split_cells_with_thin_lines_Pseudomonas_aeruginosa1(matrix,matrix_Filtered);    
...
case 39                  
if nargin==0                  
matrix(39)={'split_cells_with_thin_lines_e_coli1'}     ;             
else                  
...
case 41                  
if nargin==0                  
matrix(41)={'function 41- detect spots within cells'}     ;             
else  
